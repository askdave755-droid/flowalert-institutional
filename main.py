"""
FlowAlert Institutional Monitor - PostgreSQL Version
...
app = FastAPI(title="FlowAlert Institutional", version="2.0-pg")
...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS cot_data (
            ...
            nonrep_long INTEGER DEFAULT 0,
            nonrep_short INTEGER DEFAULT 0,
    ...
    def calculate_bias(self) -> Dict[str, Any]:
        ...
        if noncomm_extreme > 0.25 and noncomm_net_now > 0:
            if bias == "BULLISH":
                ...
                bias = "BEARISH"
                confidence = 75
                reasons.append("Specs extreme long + commercials selling")
...
@app.get("/health")
def health():
    return {"status": "ok", "system": "FlowAlert Institutional", "version": "2.0-pg", "db": "postgresql"}
    ...
    try:
        c.execute(
            """INSERT INTO cot_data 
               ...
               nonrep_long = EXCLUDED.nonrep_long,
               nonrep_short = EXCLUDED.nonrep_short,
...
def dashboard():
    ...
    <body>
        ...
        <p><a href="/health" style="color:#ffd700">/health</a> | <a href="/signal?symbol=NQ" style="color:#ffd700">/signal?symbol=NQ</a> | <a href="/bias/NQ" style="color:#ffd700">/bias/NQ</a></p>
    </body></html>
    """
